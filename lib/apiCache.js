...
class ApiCache {
  ...
  route(config = {}) {
    ...
    const { prefix, expire } = config = {
      prefix: "api-router-cache:default",
      expire: 300,
    ...
    return async (ctx, next) => {
      if (ctx.request.method === "GET") {
        ...
        cacheKey = `api-router-cache:${prefix}:${cacheKey}`;
    ...
    if (_this.redis.nodes) {
      ...
      await Promise.all(
        masters.map(async function (node) {
          const keys = await node.keys(`api-router-cache:${prefix}*`);
          if (keys.length) {
            await node.del(keys);
          }
        })
      ...
      const keys = await _this.redis.keys(`api-router-cache:${prefix}*`);
      if (keys.length) {
